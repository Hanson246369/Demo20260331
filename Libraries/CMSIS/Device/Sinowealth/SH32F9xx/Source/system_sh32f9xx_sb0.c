...
#elif defined SYSCLK_FREQ_36MHz 
    SetSysClockTo36();  // HSE:8M  RC:24M(default)   SYSCLK=PLLCLK=36M  AHB=HCLK=36M  APB0=36M  APB1=36M
#elif defined SYSCLK_FREQ_48MHz 
    SetSysClockTo48();  // HSE:8M  RC:24M(default)   SYSCLK=PLLCLK=48M  AHB=HCLK=48M  APB0=24M  APB1=24M
#elif defined SYSCLK_FREQ_56MHz 
    SetSysClockTo56();  // HSE:8M  RC:24M(default)   SYSCLK=PLLCLK=56M  AHB=HCLK=56M  APB0=28M  APB1=28M
#elif defined SYSCLK_FREQ_72MHz 
    SetSysClockTo72();  // HSE:8M  RC:24M(default)   SYSCLK=PLLCLK=72M  AHB=HCLK=72M  APB0=36M  APB1=36M
#endif
}
...
void SystemCoreClockUpdate (void)
{
    ...
    switch(hpre_div)
    {
        case 0 : SystemCoreClock = SystemCoreClock;break;
        case 1 : SystemCoreClock = SystemCoreClock / 2;break;
        case 2 : SystemCoreClock = SystemCoreClock / 4;break;
        case 3 : SystemCoreClock = SystemCoreClock / 8;break;
        case 4 : SystemCoreClock = SystemCoreClock / 16;break;
        case 5 : SystemCoreClock = SystemCoreClock / 32;break;
        default : SystemCoreClock = SystemCoreClock / 32;break;
    }
}
...
static void SetSysClockToLSE(void)
{
    ...
    SYSCFG->SAFR.V32 = 0x5AA50004;//Unlock SYSCFG SAFR register and set XTALX1 and XTALX2 pin for oscillator function
...
static void SetSysClockTo12(void)
{
    ...
    if(HSEStatus) //HSE Startup succeed,Sysclock switch to HSE
    {
        ...
        RCC->CFGR.BIT.PLLXTPRE = 0; //Pre-divider 1,PLLCLKIN is HSE_VALUE
        RCC->CFGR.BIT.PLLF = (uint8_t)(30 - 15);
        RCC->CFGR.BIT.PLLK = (uint8_t)(5 - 1); 
...
static void SetSysClockTo36(void)
{
    ...
    { 
        ...
        RCC->CFGR.BIT.PLLXTPRE = 0x02;// PREFCLK = PLLCLKIN / 3 =  HSI / 3 = 8 MHz
        RCC->CFGR.BIT.PLLF = (uint8_t)(36 - 15);
        RCC->CFGR.BIT.PLLK = (uint8_t)(4 - 1);
    ...
    RCC->CFGR.BIT.PPRE0 = 0; //Config APB0 PCLK0 Prescaler to 0,PCLK0 is 36 MHz
    RCC->CFGR.BIT.PPRE1 = 0; //Config APB1 PCLK1 Prescaler to 0,PCLK1 is 36 MHz
...
static void SetSysClockTo56(void)
{
    ...
    if(HSEStatus) //HSE Startup succeed,Sysclock switch to HSE
    {
        ...
        RCC->CFGR.BIT.PLLSRC = 1; //HSE as PLL clock source 
        RCC->CFGR.BIT.PLLXTPRE = 0;
        RCC->CFGR.BIT.PLLF = (uint8_t)(28 - 15);
    ...
    RCC->CFGR.BIT.HPRE = 0;  //Config AHB SYSCLK Prescaler to 0,HCLK is 56MHz
    RCC->CFGR.BIT.PPRE0 = 1; //Config APB0 PCLK0 Prescaler to 2,PCLK0 is 28MHz
    RCC->CFGR.BIT.PPRE1 = 1; //Config APB1 PCLK1 Prescaler to 2,PCLK1 is 28MHz
...
static void SetSysClockTo72(void)
{
    ...
    #if HIGH_SPEED_OSCILLATOR_SOURCE //No 0:use HSE=8M    0:use HSI,RC=24M
    ...
    else //HSE Startup faild,switch to HSI,HSI is 24 MHz,need PLL to generate 72 MHz Clock
    {
        ...
        //to be  calculate PLL factor 
        RCC->CFGR.BIT.PLLSRC = 0; //HSI as PLL clock source
        RCC->CFGR.BIT.PLLXTPRE = 0x02; // PREFCLK = PLLCLKIN / 3 = 8 MHz
        RCC->CFGR.BIT.PLLF = (uint8_t)(36 - 15);
        RCC->CFGR.BIT.PLLK = (uint8_t)(2 - 1);
    ...
    RCC->CFGR.BIT.HPRE = 0;  //Config AHB SYSCLK Prescaler to 0,HCLK is 72MHz
    RCC->CFGR.BIT.PPRE0 = 1; //Config APB0 PCLK0 Prescaler to 2,PCLK0 is 36MHz
