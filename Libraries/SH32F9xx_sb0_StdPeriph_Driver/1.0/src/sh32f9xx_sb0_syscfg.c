...
/**
  ...
  * @param  SYSCFG_VLVR_LEVEL_Type: Specifie the LVR voltage threshold.
  *          This parameter can be a value of @ref SYSCFG_VLVR_LEVEL_Type.
  * @retval None
  */
...
/**
  ...
  * @retval New state of BOD Flag(SH_SET or RESET).
  */
...
void SYSCFG_NMIInterruptTriggerConfig(uint16_t TriggerSource, FunctionalState NewState)
{
    ...
    SYSCFG->SAFR.V32 &= (uint32_t)0x0000FFFF; //lock
}
...
/**
  ...
  *          This parameter can be one of the @ref SYSCFG_SWJCFG_Type enum values:
  *            @arg SYSCFG_SWJ_All_Function
  *            @arg SYSCFG_SWD_CLOSE_PinRemap
  * @retval None
  */
...
/**
  ...
  * @param  OSCXPin_Mode: specifie the value for XTALX pin mode.
  *         This parameter can be one of the @ref SYSCFG_OSCXPin_Type enum values:
  *         @arg SYSCFG_OSCX_GPIO
  *         @arg SYSCFG_OSCX_Crystal
  * @retval None
  */
void SYSCFG_OSCPinConfig(SYSCFG_OSCPin_Type OSCPin_Mode, SYSCFG_OSCXPin_Type OSCXPin_Mode)
{
    uint32_t temp = 0;

    /* Check the parameter */
    assert_param(IS_SYSCFG_OSC_PIN_MODE(OSCPin_Mode));
    assert_param(IS_SYSCFG_OSCX_PIN_MODE(OSCXPin_Mode));

    temp = SYSCFG->SAFR.V32 & (~SYSCFG_SAFR_OSCCFG_Msk);
    temp &= (~SYSCFG_SAFR_OSCXCFG_Msk);

    temp |= (uint32_t)OSCPin_Mode;
    temp |= (uint32_t)(OSCXPin_Mode << 2);
    SYSCFG->SAFR.V32 = ((uint32_t)0x5AA5 << 16) | temp;
...
/**
  ...
  *          This parameter can be one of the following values:
  *            @arg SH_ENABLE  to lock the selected sectors
  *            @arg SH_DISABLE to unlock the selected sectors
  * @retval None
  */
void SYSCFG_CRAMLockConfig(uint8_t SectorNum, FunctionalState NewState)
{
    ...
    SYSCFG->CRAMLOCK.V32 &= (uint32_t)0x0000FFFF; //LOCK
}
...
/**
  * @brief  Enable or disable EFT Function.
  * @param  SYS_EFT_type: specifie the value for XTAL pin mode.
  *         This parameter can be one of the @ref SYSCFG_EFT_Type enum values:
  *         @arg SYSCFG_EFT_100M 
  *         @arg SYSCFG_EFT_72M  
  *         @arg SYSCFG_EFT_50M  
  *         @arg SYSCFG_EFT_24M  
  ...
  * @retval None
  */
void SYSCFG_EFTConfig(SYSCFG_EFT_Type SYS_EFT_type, FunctionalState NewState)
{
    uint32_t temp = 0;

    /* Check the parameters */
    assert_param(IS_SYSCFG_EFT_MODE(SYS_EFT_type));
    assert_param(IS_FUNCTION_STATE(NewState));

    temp = SYSCFG->EFTEN.V32;
    temp |= (uint32_t)(SYS_EFT_type << 1);
    if (NewState != SH_DISABLE)
    {
        temp |= SYSCFG_EFTEN_EFT_EN_Msk;
    }
    else
    {
        temp &= (~SYSCFG_EFTEN_EFT_EN_Msk);
    }
    SYSCFG->DBGCR.V32 = temp;
}
